...
def handle_backup_slot(args):
    ...
    if not os.path.exists(part_path):
        print(f"Error: Slot '{slot}' not found. Was the system installed with obsidianctl?", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        if os.path.exists(pacman_dir):
            metadata["packages"] = [d for d in os.listdir(pacman_dir) if os.path.isdir(os.path.join(pacman_dir, d))]
